...
run_in_openroad "./openroad_docker_lab/scripts/setup_demo.sh"

echo "[3/7] Running synthesis"
FLOW_VARIANT="${FLOW_VARIANT}" \
  "${ROOT_DIR}/openroad_docker_lab/scripts/run_orfs.sh" \
  designs/nangate45/macro_cluster_demo/config.mk synth

echo "[4/7] Running floorplan and placement"
"${ROOT_DIR}/openroad_docker_lab/scripts/floorplan_demo.sh" "${FLOW_VARIANT}"

echo "[5/7] Preparing flow.py input directory"
run_in_openroad "./openroad_docker_lab/scripts/export_flow_inputs.sh \
...
run_in_openroad "python3 openroad_docker_lab/scripts/fix_plc_meta.py \
