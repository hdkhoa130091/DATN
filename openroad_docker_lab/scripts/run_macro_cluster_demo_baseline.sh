...
if [ "${RUN_PIPELINE}" = "1" ]; then
  "${ROOT_DIR}/openroad_docker_lab/scripts/run_macro_cluster_demo_pipeline.sh"
fi
...
if [ ! -f "${NETLIST_PATH}" ] || [ ! -f "${PLC_PATH}" ]; then
  ...
  echo "  ./openroad_docker_lab/scripts/run_macro_cluster_demo_pipeline.sh"
  exit 1
fi
...
python3 "${ROOT_DIR}/openroad_docker_lab/scripts/summarize_macro_cluster_baseline.py" \
  --root "${RESULT_ROOT}" \
